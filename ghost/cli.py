...
from ghost.console import Colors, Console, GhostSpinner, Icons, SpinnerStyle
...
@click.option(
    "--provider",
    "-p",
    type=click.Choice(["groq", "openai", "ollama", "anthropic", "openrouter", "auto"]),
    default="auto",
...
def init(path: str, provider: str, model: Optional[str], framework: str):
    ...
    if ghost_file.exists():
        if not click.confirm(
            f"  {Icons.WARNING} ghost.toml already exists. Overwrite?", default=False
        ):
            Console.info("Initialization cancelled")
            return

    # Provider selection
    from ghost.providers import PROVIDER_MODELS, auto_detect_provider
    ...
    for i, p in enumerate(providers_list, 1):
        marker = (
            f"{Colors.GREEN}●{Colors.RESET}" if p == provider else f"{Colors.DIM}○{Colors.RESET}"
        )
        desc = {
        ...
        Console.print(
            f"    {marker} {i}. {Colors.CYAN}{p}{Colors.RESET} {Colors.DIM}- {desc}{Colors.RESET}"
        )
    ...
    if env_var_name:
        ...
        if existing_key:
            masked = (
                existing_key[:8] + "..." + existing_key[-4:] if len(existing_key) > 12 else "***"
            )
            Console.success(f"Found {env_var_name}: {masked}")
            if not click.confirm(f"  {Icons.ARROW} Use existing key?", default=True):
                api_key = click.prompt(
                    f"  {Icons.ARROW} Enter your {provider.upper()} API key", hide_input=True
                )
        else:
            ...
            if api_key:
                Console.info("Your API key will be saved to .env file")
    ...
    if available_models:
        ...
        for m in available_models[:6]:  # Show top 6
            marker = (
                ...
                if m == default_models.get(provider)
    ...
    if not model:
        model = click.prompt(
            f"  {Icons.ARROW} Enter model name", default=default_models.get(provider, "auto")
        )
    ...
    with GhostSpinner(
        "Creating configuration", style=SpinnerStyle.DOTS, color=Colors.CYAN
    ) as spinner:
        try:
    ...
    with GhostSpinner(
        "Scanning project files", style=SpinnerStyle.DOTS2, color=Colors.MAGENTA
    ) as spinner:
        try:
    ...
    Console.divider()
    Console.print(
        f"  {Colors.DIM}Provider:{Colors.RESET}  {Colors.BRIGHT_WHITE}{provider}{Colors.RESET}"
    )
    Console.print(
        f"  {Colors.DIM}Model:{Colors.RESET}     {Colors.BRIGHT_WHITE}{model}{Colors.RESET}"
    )
    Console.print(
        f"  {Colors.DIM}Framework:{Colors.RESET} {Colors.BRIGHT_WHITE}{framework}{Colors.RESET}"
    )
    Console.print(
        f"  {Colors.DIM}Files:{Colors.RESET}     {Colors.BRIGHT_WHITE}{file_count}{Colors.RESET}"
    )
    Console.divider("═")
    Console.newline()
    ...
    Console.print(
        f"  {Colors.CYAN}ghost generate <file>{Colors.RESET}  Generate tests for a specific file"
    )
    Console.newline()
...
def watch(path: str, verbose: bool):
    ...
    try:
        import ghost.main as ghost_main
...
def _check_daemon(project_root: Path) -> Optional[int]:
    ...
    """
    from ghost.daemon import check_pid
...
@click.argument("path", type=click.Path(), default=".")
@click.option(
    "--detach/--foreground", "detach", default=True, help="Run in background (default: detach)"
)
def start(path: str, detach: bool):
    ...
    if detach:
        ...
        if existing_pid is not None:
            ...
            Console.info("Use 'ghost stop' to stop it first, or 'ghost logs' to view output")
            return
...
def stop(path: str):
    ...
    with GhostSpinner(
        "Waiting for daemon to stop", style=SpinnerStyle.DOTS, color=Colors.YELLOW
    ) as spinner:
...
def logs(path: str, follow: bool, lines: int):
    ...
    if follow:
        ...
        try:
            ...
            Console.newline()
            Console.info("Run 'ghost logs -f' to follow in real-time")
...
def generate(file: str, output: Optional[str], force: bool):
    ...
    # Generate tests
    with GhostSpinner(
        "Generating tests with AI", style=SpinnerStyle.DOTS, color=Colors.CYAN
    ) as spinner:
        try:
            ...
            test_code = generator.get_test_code(
                source_code=source_code, source_path=str(project_root), filename=file_path.name
            )
    ...
    with GhostSpinner("Running pytest", style=SpinnerStyle.DOTS2, color=Colors.YELLOW) as spinner:
        try:
            from ghost.runner import run_test
...
@click.option(
    "--set-provider",
    type=click.Choice(["groq", "openai", "ollama", "anthropic", "openrouter"]),
...
def config(
    show: bool, set_provider: Optional[str], set_model: Optional[str], set_api_key: Optional[str]
):
    """
...
def _show_config(project_root: Optional[Path]):
    ...
    Console.print(
        f"  {Colors.DIM}Project:{Colors.RESET}    {Colors.BRIGHT_WHITE}{project_root.name}{Colors.RESET}"
    )
    Console.print(
        f"  {Colors.DIM}Config:{Colors.RESET}     {Colors.BRIGHT_WHITE}{config_path}{Colors.RESET}"
    )
    Console.newline()
...
def _update_config(project_root: Path, provider: Optional[str] = None, model: Optional[str] = None):
    ...
    if provider:
        ...
        content = re.sub(r'provider\s*=\s*"[^"]*"', f'provider = "{provider}"', content)
...
def providers(check: bool):
    ...
    from ghost.providers import POPULAR_MODELS, list_available_providers
    ...
    _print_provider_status(
        "Ollama", status.get("ollama", False), "ollama serve", "http://localhost:11434"
    )
    _print_provider_status(
        "LM Studio", status.get("lmstudio", False), "Start LM Studio", "http://localhost:1234"
    )
    ...
    _print_provider_status(
        "OpenRouter", status.get("openrouter", False), "OPENROUTER_API_KEY", "Pay per use"
    )
    ...
    for name, model in list(POPULAR_MODELS.items())[:10]:
        ...
        Console.print(
            f"  {Colors.BRIGHT_WHITE}{name:<25}{Colors.RESET} {provider:<12} {Colors.DIM}{model.description}{Colors.RESET}"
        )
    ...
    Console.print("  ollama serve  # Start server")
    Console.print("  ollama pull llama3.2  # Download model")
    Console.newline()
...
def doctor():
    ...
    # Check AI providers
    from ghost.providers import list_available_providers
