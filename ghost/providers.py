...
from ghost.console import Console
from ghost.rate_limiter import RateLimiter, call_with_retry
...
POPULAR_MODELS: Dict[str, ModelConfig] = {
    ...
    "llama-3.3-70b": ModelConfig(
        "llama-3.3-70b-versatile", ProviderType.GROQ, 131072, True, 30, "Fast Llama 3.3 70B on Groq"
    ),
    "llama-3.1-8b": ModelConfig(
        "llama-3.1-8b-instant", ProviderType.GROQ, 131072, True, 30, "Fast Llama 3.1 8B on Groq"
    ),
    "mixtral-8x7b": ModelConfig(
        "mixtral-8x7b-32768", ProviderType.GROQ, 32768, True, 30, "Mixtral MoE on Groq"
    ),
    "gemma2-9b": ModelConfig(
        "gemma2-9b-it", ProviderType.GROQ, 8192, True, 30, "Google Gemma 2 9B on Groq"
    ),
    # OpenAI models
    "gpt-4o": ModelConfig(
        "gpt-4o", ProviderType.OPENAI, 128000, True, 500, "GPT-4o - Latest OpenAI flagship"
    ),
    "gpt-4o-mini": ModelConfig(
        "gpt-4o-mini", ProviderType.OPENAI, 128000, True, 500, "GPT-4o Mini - Fast & cheap"
    ),
    "gpt-4-turbo": ModelConfig(
        "gpt-4-turbo", ProviderType.OPENAI, 128000, True, 500, "GPT-4 Turbo"
    ),
    "gpt-3.5-turbo": ModelConfig(
        "gpt-3.5-turbo", ProviderType.OPENAI, 16385, True, 3500, "GPT-3.5 Turbo - Fast"
    ),
    # Anthropic models
    ...
    "claude-3-haiku": ModelConfig(
        "claude-3-haiku-20240307", ProviderType.ANTHROPIC, 200000, True, 50, "Claude 3 Haiku - Fast"
    ),
    # Ollama local models
    "ollama/llama3.2": ModelConfig(
        "llama3.2", ProviderType.OLLAMA, 131072, True, 999, "Llama 3.2 (Local)"
    ),
    "ollama/codellama": ModelConfig(
        "codellama", ProviderType.OLLAMA, 16384, True, 999, "CodeLlama (Local)"
    ),
    "ollama/deepseek-coder": ModelConfig(
        "deepseek-coder-v2", ProviderType.OLLAMA, 128000, True, 999, "DeepSeek Coder V2 (Local)"
    ),
    "ollama/qwen2.5-coder": ModelConfig(
        "qwen2.5-coder", ProviderType.OLLAMA, 32768, True, 999, "Qwen 2.5 Coder (Local)"
    ),
    "ollama/mistral": ModelConfig(
        "mistral", ProviderType.OLLAMA, 32768, True, 999, "Mistral 7B (Local)"
    ),
    # OpenRouter (access many models)
    "openrouter/auto": ModelConfig(
        "openrouter/auto", ProviderType.OPENROUTER, 128000, True, 60, "Auto-select best model"
    ),
}
...
class OpenAIProvider(BaseProvider):
    ...
    @call_with_retry(max_retries=5, base_delay=2.0)
    def chat(
        self, messages: List[Dict[str, str]], model: str = "gpt-4o-mini", temperature: float = 0.1
    ) -> str:
        RateLimiter.wait()
        response = self.client.chat.completions.create(
            messages=messages,
...
class OllamaProvider(BaseProvider):
    ...
    def _create_client(self):
        ...
        return OpenAI(
            api_key="ollama", base_url=f"{self.base_url}/v1"  # Ollama doesn't need a real key
        )

    @call_with_retry(max_retries=3, base_delay=1.0)
    def chat(
        self, messages: List[Dict[str, str]], model: str = "llama3.2", temperature: float = 0.1
    ) -> str:
...
class LMStudioProvider(BaseProvider):
    ...
    def _create_client(self):
        ...
        return OpenAI(api_key="lm-studio", base_url=self.base_url)

    @call_with_retry(max_retries=3, base_delay=1.0)
    def chat(
        self, messages: List[Dict[str, str]], model: str = "local-model", temperature: float = 0.1
    ) -> str:
        response = self.client.chat.completions.create(
            messages=messages,
            model=model,
